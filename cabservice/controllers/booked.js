...
exports.getBooked = function(req, res) {
    ...
    Booked.find({}).lean().exec(function(err, booked) {
        if(err){
            res.send(err);
            console.log(err);
            console.log(booked);
        ...
        console.log("Booked Location Lat: "+booked.location_lat);
        console.log("Booked Location Long: "+booked.location_long);
        ...
        //username.
        for (var i = 0; i < l; i++) {
            User.findById(booked[i].userId, closure(booked[i]));
        }
    });
};
